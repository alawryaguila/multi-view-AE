...
class VAE_classifier(pl.LightningModule, Optimisation_VAE):
    ...
    def __init__(
        ...
        self.classifier = Classifier(input_dim=self.z_dim, hidden_layer_dims=classifier_layer_dims, output_dim=self.n_labels, non_linear=self.non_linear_classifier)

    def configure_optimizers(self):
        optimizers = [torch.optim.Adam(list(self.encoders[i].parameters()) + list(self.decoders[i].parameters()),
                                      lr=self.learning_rate) for i in range(self.n_views)] + [torch.optim.Adam(list(self.classifier.parameters()),
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        self.log(f'train_ce_loss', loss['ce'], on_epoch=True, prog_bar=True, logger=True)
        return loss['total']
